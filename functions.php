...
if ( ! function_exists( 'dog_setup' ) ) :
	...
	function dog_setup() {
		 ...
		 * If you're building a theme based on dog, use a find and replace
		 * to change 'dog' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'dog', get_template_directory() . '/languages' );
		...
		add_theme_support( 'custom-background', apply_filters( 'dog_custom_background_args', array(
 ...
 */
function dog_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'dog_content_width', 640 );
}
add_action( 'after_setup_theme', 'dog_content_width', 0 );
 ...
 */
function dog_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'dog' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'dog' ),
...
add_action( 'widgets_init', 'dog_widgets_init' );
...
function dog_scripts() {
	...
	wp_enqueue_script( 'dog-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'dog-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
...
add_action( 'wp_enqueue_scripts', 'dog_scripts' );
...
function universal_send() {
    ...
    if ( check_ajax_referer( 'NEHERTUTLAZIT', 'nonce', false ) ) {
      ...
      $headers = array(
        'From: Питомник Курска <noreply@propuska-mkad-ttk-sk.ru>',
